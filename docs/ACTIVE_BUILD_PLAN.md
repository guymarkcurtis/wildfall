...
- [x] Add an `Interactable` runtime interface to placed buildings:
      `can_interact(player)`, prompt metadata, and open/close handling on
      `Building` (backed by the authored `InteractionProfile`; the panel
      pipeline lives in the manager, so interact/close have one owner) — no
      type/name branches.
- [x] Add an `InteractionManager` under Main as the single owner of current
...
- [x] Insert the router into `Player._handle_interaction()` after cave
      ...
      E with a panel open closes it and is consumed.
- [x] Add an `InteractablePanel` scene/script: title, close button, backdrop,
...
- [x] Extract one shared slot-grid control (`StorageGridView`); player,
      ...
      (right click → half), read-only grids for output slots, tooltips.
- [x] Define and implement close behaviour for: Escape, close button,
      opening another object ("switched"), out of range, entering a cave,
      loading/new seed ("world_reset"), building removal/damage, death,
      pause (title transition frees the scene). Each safe when fired twice —
      the close pipeline early-returns without emitting when already closed.
- [x] Accessibility path: focus stays in the panel (backdrop is
      MOUSE_FILTER_STOP so world clicks never leak), Escape always closes the
      open panel and is consumed, movement/firing/building locked while open.
- [x] Tests: targeting priority, prompt visibility, input lock, every close
      reason (fired twice for idempotence), direct object-to-object
      switching, and a runtime-registered `test_crate` fixture (not the
      chest) proving the router is generic.
      (`tests/test_interaction_router.gd`, 25 checks; stairs 37/0; placement
      55/0; content 69/0; full harness 440/0; sandbox 0 failures; live boot
      0 script errors.)
...
- [x] Interaction highlight/focus, concise prompts, capacity/fuel errors,
      craft success/failure feedback, animation readable at 32 px.
- [x] Small Building Sandbox tutorial card: layers, orientation, construction
...
| 2026-09-14 | M9 box 7: the manager owns a generic focus frame (four 2 px polygon strips on the target tile, one render band above the target, hidden when nothing is targetable or while a panel is open) and one toast lane on the panel (`toast_requested` → parent HUD toast) that carries craft success, data-tableled craft failures, and per-grid transfer rejections with fuel-tag-aware wording; interaction transitions (opening/closing states at authored fps) run through token-guarded `Building` coroutines, and `set_crafting_active(false)` no longer snaps the resting state while an authored interaction transition owns the presentation; the Building Sandbox tutorial card is a data resource (`SandboxTutorialCard`: ordered heading→one-liner lines) plus a top-right panel auto-shown at sandbox start, re-opened from the drawer's "?" button. | The plan's exit is "understandable without developer knowledge; animation readable at 32 px": a generic focus frame keeps the manager free of per-part branching (any future interactable gets it for free), the single toast lane keeps every refusal legible in the one place the player is looking, and the card keeps sandbox onboarding in content rather than code. The same-tick close bug was a real presentation regression the M5 close-reason tests could not catch — they assert state, not frames: `close()` ran the interaction close and the crafting deactivation in one tick, so the unconditional resting snap clobbered "closing" before its first frame rendered. Profiles with empty interaction transitions (workbench/furnace/torch) are unaffected by the guard, so only the chest's authored closing frame (1 frame @ 8 fps) changes behaviour. |
