...
- [ ] Replace the `Vector3i(x, y, story) -> Building` single-slot model with a
...
- [ ] Keep a compatibility API for `get_building_at()` while migrating
      callers; add precise layer/edge query APIs; retire ambiguous calls
      deliberately.
- [ ] Refactor placement, ghost preview, demolition, health/destroy
      callbacks, selection, proximity checks, and serialization to operate on
      complete records/footprints transactionally.
- [ ] Implement floor + edge + object + overhead coexistence, wall→door/
      window edge replacement where data allows, multi-tile object
      reservations, and exact refund handling.
- [ ] Implement the conservative support validator (upper floors need
...
- [ ] **Bump save format to v8**: building entries become layered records
      ...
      keep loading.
- [ ] Update `docs/SAVE_FORMAT.md` with the v8 schema and migration.
- [ ] Tests: same tile/story accepts floor+object+overhead and rejects
