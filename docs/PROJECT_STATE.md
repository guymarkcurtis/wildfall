...
Automated headless run of the real main scene — **152/152 checks passed,
0 failures**:
...
| Crafting + research (starting recipes visible; locked stone recipes unlock after paid research) | PASS |
...
| Building (palette selection, HUD click pass-through, 4-story support/cutaway) | PASS |
...
| Texture packs (stock-card export, structured image metadata, editable pack creation, live switch, fallback) | PASS |

A 30-second headless run of the actual game also completed with 0 errors,
0 warnings and 0 leaked objects.

### Known Issues (remaining)
1. Recipe list has no scroll view — with 56 recipes the list overflows the panel bounds (functional, cosmetic).
2. Crafting stations (campfire/furnace/anvil) are data-only: recipes show a station but proximity is not enforced because stations are not placeable yet.
3. Some future-phase scripts remain intentionally unwired; see the inventory in `docs/ARCHITECTURE.md`.
...
- **Crafting System**: 56 recipes; every unlocked recipe has an obtainable ingredient chain. The panel hides research-locked recipes and retains the obtainability filter as a safety net.
...
- `src/ui/` — HUD, DebugOverlay, InventoryPanel, InventorySlot, BuildPalette, TechnologyPanel, CraftingPanel
- `src/systems/` — SaveSystem, ItemDatabase, BuildingManager, TechnologySystem, TexturePackManager, CreatureSpawner (Phase 3)
...
- `tests/` — `test_game.gd` headless harness (152 checks)
...
1. Make crafting stations placeable and enforce their proximity requirements
2. Add a scroll view to the recipe list (panel currently overflows)
3. Persist destroyed resources, creatures, and buildings in saves
4. Polish building/creature/player sprites (terrain/resource atlases already wired)
