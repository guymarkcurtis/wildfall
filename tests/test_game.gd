...
func _run_checks() -> void:
	...
	var inv_panel = main.get_node_or_null("HUD/InventoryPanel")
	bus.toggle_inventory_ui.emit()
	_check(inv_panel != null and inv_panel.is_open() == true, "I while the journal is open switches to the inventory")
	_check(mission_panel.visible == false, "Overlay menus replace each other: opening the inventory closes the journal")
	bus.toggle_inventory_ui.emit()
	_check(inv_panel.is_open() == false, "I again closes the inventory")
	_check(mission_panel.visible == false, "Closing the active menu returns to the game (no earlier menu reopens)")
	bus.toggle_missions_ui.emit()
	_check(mission_panel.visible == true, "J re-opens the journal after the swap")
	bus.toggle_missions_ui.emit()
